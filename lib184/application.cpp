...
int runApplication(Application &app, int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++) {
		...
		if (index == std::string::npos) {
			...
			app.options.emplace(param.substr(0, index),
								param.substr(index + 1));
		}
	}
	...
	app.window = glfwCreateWindow(app.width, app.height, "", NULL, NULL);
	if (!app.window) {
		return -1;
	}
	...
	glfwSetWindowTitle(app.window, app.name.data());
