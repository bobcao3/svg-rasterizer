...
vec3 Texture::sample(const SampleParams &sp)
{
...
vec3 Texture::sample_nearest(vec2 uv, int level)
{
	// Optional helper function for Parts 5 and 6
...
vec3 Texture::sample_bilinear(vec2 uv, int level)
{
	// Optional helper function for Parts 5 and 6
	// Feel free to ignore or create your own
	return vec3();
...
void Texture::generate_mips(int startLevel)
{
	...
	for (int i = 1; i <= numSubLevels; i++) {
		...
		level.width = width;
		level.height = height;
		level.texels = std::vector<unsigned char>(3 * width * height);
	}
		...
		if (currLevel.height == prevLevel.height) {
			...
			for (int j = 0; j < currLevel.height; j++) {
				...
				for (int i = 0; i < currLevel.width; i++) {
					...
					// convert back to format of the texture
					float_to_uint8(currLevelMem + currLevelPitch * j + 3 * i,
								   result);
				}
			}
		}
