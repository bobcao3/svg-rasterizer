...
float area(const vector<vec2> &contour) {
...
bool snip(const vector<vec2>& contour,int u,int v,int w,int n,int *V ) {
...
void triangulate(const Polygon& polygon, vector<vec2>& triangles) {
  
  const vector<vec2>& contour = polygon.points;
